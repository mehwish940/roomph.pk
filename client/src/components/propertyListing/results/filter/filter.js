...
function CustomizedSlider() {
  ...
  return (
    <Box sx={{ width: 320 }}>
      ...
      <hr className="dottedLine p-0" style={{ display: 'inline-block', width: '140px', borderTop: '1px dotted #F53500' }} />
...
class BootstrapModal extends React.Component {
  ...
  render() {
    return (
      <div>
        <button className='filterBtn' onClick={() => this.handleModalShowHide()}>
          Filter
        </button>
        <Modal show={this.state.showHide} centered>
          <button onClick={() => this.handleModalShowHide()} className="mt-2 ml-2" style={{ border: "none", background: "none", paddingRight: '400px' }}><MdOutlineKeyboardArrowLeft /></button>
          <div className="mx-auto m-0 p-0">
            <p className='m-0 p-0 text-center' style={{ fontFamily: 'Gotham Rounded Bold' }}>Price per night</p>
            <CustomizedSlider />
          </div>
          <div className="mt-2 mb-3" style={{ marginLeft: '15px' }}>
            <Checkboxes />
          </div>
          <button className="dateDoneBtn mx-auto mb-3" onClick={() => {
            history.push(`/propertylisting/${this.props.city}/${this.props.checkIn}/${this.props.checkOut}/${this.props.adults}/${this.props.rooms}/${this.props.nights}/${priceStart}/${priceEnd}/${premium}/${rating}/${category}`); this.handleModalShowHide(); setTimeout(() => {
              this.props.setStateOfParentloadingFilter()
            }, 1000);
          }}>Done</button>
        </Modal>
